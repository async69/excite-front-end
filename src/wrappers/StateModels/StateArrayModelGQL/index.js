...
export default class StateArrayModel {
  ...
  setExceptionMappers = (exceptions) => {
    this.exceptionMappers = exceptions
  }
  ...
  createSlice = () => {
    ...
    const body = {
      fetchBody: this.fetchBody,
      editBody: this.editBody,
      addBody: this.addBody,
      deleteBody: this.deleteBody
    }

    const isAddReverse = this.addReverse
    const exceptionMappers = this.exceptionMappers
    this.core.slice = createSlice({
      ...
      reducers: {
        ...
        successFetch(state, action) {
          ...
          const exceptionKeys = Object.keys(exceptionMappers)
          const currentActions = Object.keys(action.payload.data)
          const currentAction = currentActions[0]
          const exceptionIndex = exceptionKeys.findIndex(key => String(key) === String(currentAction))
          let response = {}
          if (exceptionIndex >= 0) {
            response = data.data[currentAction]
          } else {
            response = data.data[body.fetchBody]
          }
        ...
        successAdd(state, action) {
          if (action.payload.isMock) {
            ...
            const response = addMapper(action.payload);
            if (isAddReverse) {
              state.data.unshift(response.data[body.addBody]);
            } else {
              state.data.push(response.data[body.addBody]);
            }
            state.addStatus = {
              ...STATE_OK,
              response: action.payload
        ...
        successEdit(state, action) {
          ...
          if (exceptionIndex >= 0) {
            ...
            response = data.data[body.editBody]
          }
        ...
        successDelete(state, action) {
          ...
          if (deleteEnabled) {
            ...
            const response = action.payload.data[body.deleteBody]
            state.data = state.data.filter(
              element => element._id !== response._id
            );
          }
  ...
  ActionSuccessAdd = data => {
    const { successAdd } = this.getActions();
    return {
      special: true,
      ...successAdd,
      payload: data
  ...
  getAPIHandles = () => {
    return {
      Add: (body, isMock = this.config.isMock, config = this.config) => {
        const { requestAdd, successAdd, failureAdd } = this.getActions();
        this.postReqest = {
          ...this.config,
          ...this.postReqest,
      ...
      Fetch: (body, isMock = this.config.isMock, config = this.config) => {
        const { requestFetch, successFetch, failureFetch } = this.getActions();
      ...
      Edit: (body, isMock = this.config.isMock, config = this.config) => {
        const { requestEdit, successEdit, failureEdit } = this.getActions();
        this.putRequest = {
          ...this.config,
          ...this.postReqest,
      ...
      Remove: (body, isMock = this.config.isMock, config = this.config) => {
        const {
          requestDelete,
          successDelete,
          failureDelete
